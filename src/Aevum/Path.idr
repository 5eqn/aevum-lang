...
||| ```
||| test : Path [Context]
||| test = let path1 = cons1 |*| \x => cons2 x |*| \y => |-| ctx |+| test |+| test in
|||        let path2 = cons3 |*| \z => |-| ctx in
|||        path1 |/| path2
||| ```
||| This path will try `path1` first, going through `cons1` and `cons2 x`. 
||| Upon success, `|-| ctx |+| test |+| test` will be evaluated.
||| Results of `|-| ctx |+| test` will be composed first because `|+|` is `infixl`,
...
||| Syntactic suger of `|*|` with function construction.
public export
